...
export default function Inbox() {
  const { dadosPromise, agenciasPromise, pastasPromise, nomePasta } = useLoaderData<typeof loader>();
            ...
            <Await resolve={dadosPromise}>
              {(resultado: any) => (
                <>
                  ...
                  <button onClick={() => exportarExcel(resultado.data)} className="flex-1 xl:flex-none flex items-center justify-center gap-2 px-6 py-3.5 bg-emerald-600 text-white rounded-2xl font-bold text-sm shadow-lg shadow-emerald-500/20"><Download size={18} /> Exportar</button>
                </>
              )}
            </Await>
          </Suspense>
