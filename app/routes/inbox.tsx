...
// shouldRevalidate removido para garantir a revalidação dos dados do loader ao navegar
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  switch (intent) {
    case "upload": {
      ...
      return await OperacaoService.processarPlanilha(buffer, file.name, userName);
    }
    case "update": {
    ...
    case "bulkMove": {
      ...
      const res = await OperacaoService.bulkActionPasta(ids, pastaId, filters);
      return { ...res, intent: "bulkMove" };
    }
    case "bulkDelete": {
      const ids = JSON.parse(formData.get("ids") as string).map(Number);
      const filters = JSON.parse(formData.get("filters") as string);
      const res = await OperacaoService.bulkDelete(ids, filters);
      return { ...res, intent: "bulkDelete" };
    }
...
export default function Inbox() {
  ...
  const excluirSelecionados = (totalFiltro: number) => {
    confirm({
      ...
      message: `Você está prestes a excluir ${selecionados.size || totalFiltro} itens. Esta ação não pode ser desfeita.`,
      variant: 'danger',
      onConfirm: () => {
        const formData = new FormData();
        formData.append("intent", "bulkDelete");
        formData.append("ids", JSON.stringify(Array.from(selecionados)));
        formData.append("filters", JSON.stringify(Object.fromEntries(new URLSearchParams(location.search))));
        fetcher.submit(formData, { method: "post" });
        setSelecionados(new Set());
      }
    });
        ...
        <div className="flex flex-wrap gap-4 items-center border-t border-slate-100 dark:border-slate-800 pt-2.5">
          <div className="flex items-center gap-2">
            ...
            <select value={filtros.status} onChange={e => setFilters(p => ({...p, status: e.target.value}))} className="w-36 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1 text-[10px] font-black outline-none">
              ...
              {["PENDENTE", "ANEXADO", "DIVERGENTE", "ILEGIVEL", "POSTO", "MDF EM ABERTO", "COMPLEMENTAR", "1° PERNA", "SINISTRO", "DESACORDO", "CARGA RECUSADA"].map(opt => <option key={opt} value={opt}>{opt}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-2">
