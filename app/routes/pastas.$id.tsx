...
import { 
  ...
  Trash2, UploadCloud, Loader2, Folder
} from "lucide-react";
  ...
  const moverParaPasta = (pId: number | null, pNome: string) => {
    ...
    confirm({
      ...
      onConfirm: () => {
        ...
        showToast(`${idsCount} itens movidos com sucesso!`);
      }
    });
  };
  ...
  const excluirSelecionados = () => {
    ...
    confirm({
      ...
      onConfirm: () => {
        ...
        fetcher.submit(formData, { method: "post" });
        setSelecionados(new Set());
        showToast(`${idsCount} itens excluídos.`, "error");
      }
    });
  };
    ...
    <div className="flex-1 flex flex-col min-h-0 space-y-4 animate-in fade-in duration-500">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-[2rem] shadow-sm flex flex-col xl:flex-row gap-4 items-center justify-between shrink-0">
        ...
        <div className="relative flex-1 w-full">
          ...
          <input placeholder="Busca Geral" value={filtros.search} onChange={(e) => setFilters(p => ({ ...p, search: e.target.value }))} className="w-full bg-slate-100/50 dark:bg-slate-800/50 border-0 rounded-2xl pl-12 pr-4 py-3.5 outline-none focus:ring-2 focus:ring-indigo-500/50 text-base font-medium" />
        </div>
